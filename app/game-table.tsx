...
import { PowerUpIcon, PowerUpMeter, type PowerUpId } from "./power-up-icons";
import { Ban, ChevronLeft, ChevronRight, Clock3, Download, HandHeart, Music2, Pause, Play, Radio, RotateCcw, ShieldCheck, Target, Volume2, VolumeX, Zap } from "lucide-react";
...
type LeadFlash = { id: number; playerId: number };

type AudioWindow = Window & { webkitAudioContext?: typeof AudioContext };

const GAIN_NOTES = [523.25, 659.25, 783.99, 880], GREATER_GOOD_NOTES = [392, 349.23, 329.63, 293.66, 261.63];

const actionMeta: Record<ActionId, { label: string; hint: string }> = {
  smash: { label: "SMASH", hint: "Race for the pot" }, double: { label: "DOUBLE DOWN", hint: "Last-place comeback" },
  lockdown: { label: "LOCKDOWN", hint: "Freeze the table · 6s" },
  steal: { label: "STEAL", hint: "Take their next win · 15s" }, good: { label: "GREATER GOOD", hint: "Drain cash into the pot" }, shield: { label: "SHIELD", hint: "Block all attacks · 4s" },
};

const onboardingSlides: { action: PowerUpId; title: string; eyebrow: string; copy: string }[] = [
  { action: "lockdown", title: "Lock the Table", eyebrow: "LOCKDOWN", copy: "Freeze every action for 6 seconds and clear all banked Steals and active Greater Good drains. Shields survive." },
  { action: "steal", title: "Set an Ambush", eyebrow: "STEAL", copy: "Choose a rival. For 15 seconds, their next successful Smash payout is rerouted to you." },
  { action: "good", title: "Feed the Pot", eyebrow: "GREATER GOOD", copy: "Choose a rival and drain their stored cash into the shared pot over five timed payments." },
  { action: "shield", title: "Raise Your Guard", eyebrow: "SHIELD", copy: "Block every incoming attack for 4 seconds and end Greater Good drains already targeting you." },
];

const moneyDelta = (n: number) => `${n >= 0 ? "+" : "−"}$${Math.abs(n).toFixed(Math.abs(n) < 10 ? 2 : 0)}`;
const buttonFromTarget = (target: EventTarget | null) => target instanceof Element ? target.closest<HTMLButtonElement>("button") : null;

const SCORE_PULSE_MS = 2800;

export type RemoteGame = { game:Game; goal:number; seat:number; now:number; pausedAt:number|null; isHost:boolean; connected:boolean; started:boolean; send:(type:string,extra?:Record<string,unknown>)=>void; downloadLog:()=>void };

type ActionFeedProps = { game: Game; goal: number; now: number };

// Kept as a reusable view for a future History button or drawer.
export function ActionFeed({ game, goal, now }: ActionFeedProps) {
  return <aside className="action-log"><span className="eyebrow">LIVE ACTION FEED</span>{game.greaterGood.length > 0 && <div className="active-effects"><b>ACTIVE GREATER GOOD</b>{game.greaterGood.map(effect => <span key={effect.id}><HandHeart size={13}/>{game.players[effect.ownerId].name} → {game.players[effect.targetId].name} · {money(effect.moved)} moved · {money(effect.remaining * goal * GOOD_PERCENT_PER_TICK)} left · next {seconds(effect.nextTick - now)}</span>)}</div>}<div>{game.log.filter(entry => entry.type !== "network_sample" && entry.type !== "bot_decision" && entry.type !== "action_press" && entry.type !== "greater_good_tick").slice(-12).reverse().map((entry, i) => <p key={`${entry.at}-${i}`}>{entry.message}</p>)}</div></aside>;
}
...
export default function GameTable({remote}:{remote?:RemoteGame} = {}) {
  ...
  const game=remote?.game??localGame,goal=remote?.goal??localGoal,now=remote?.now??localNow,started=remote?remote.started:localStarted,pausedAt=remote?remote.pausedAt:localPausedAt,youId=remote?.seat??0;
  useEffect(() => { setOnboardingSlide(0); }, [game.sessionId, game.gameNumber]);
  const [scorePulses, setScorePulses] = useState<ScorePulse[]>([]), [leadFlash, setLeadFlash] = useState<LeadFlash | null>(null), [leaderId, setLeaderId] = useState<number | null>(null), [soundEnabled, setSoundEnabled] = useState(true);
  const sequence = useRef(0), scoreSequence = useRef(0), leadSequence = useRef(0), scoreGame = useRef(`${game.sessionId}:${game.gameNumber}`), leaderGame = useRef(`${game.sessionId}:${game.gameNumber}`), currentLeader = useRef<number | null>(null), audioLogCursor = useRef(game.log.length), previousStashes = useRef(game.players.map(player => player.stash)), awardAudioContext = useRef<AudioContext | null>(null), drainAudioContext = useRef<AudioContext | null>(null), gameRef = useRef(game), startedRef = useRef(false), pausedRef = useRef<number | null>(null), scheduledLockdown = useRef(0); useEffect(() => { gameRef.current = game; }, [game]);
  ...
  const playLeadChange = useCallback(() => {
    ...
    const context = ensureAudio("award"); if (!context) return;
    const start = context.currentTime + .008;
    [659.25, 783.99, 987.77].forEach((hz, i) => {
      const at = start + i * .14, oscillator = context.createOscillator(), volume = context.createGain();
      oscillator.type = "sine"; oscillator.frequency.setValueAtTime(hz, at);
      volume.gain.setValueAtTime(.0001, at); volume.gain.exponentialRampToValueAtTime(.055, at + .008);
      volume.gain.exponentialRampToValueAtTime(.0001, at + .18);
      oscillator.connect(volume).connect(context.destination); oscillator.start(at); oscillator.stop(at + .2);
    });
  }, [ensureAudio, soundEnabled]);
  const playButtonPress = useCallback(() => {
    if (!soundEnabled) return;
    const context = ensureAudio("award"); if (!context) return;
    const start = context.currentTime + .004, oscillator = context.createOscillator(), volume = context.createGain();
    oscillator.type = "triangle"; oscillator.frequency.setValueAtTime(260, start); oscillator.frequency.exponentialRampToValueAtTime(150, start + .07);
    volume.gain.setValueAtTime(.0001, start); volume.gain.exponentialRampToValueAtTime(.045, start + .004); volume.gain.exponentialRampToValueAtTime(.0001, start + .085);
    oscillator.connect(volume).connect(context.destination); oscillator.start(start); oscillator.stop(start + .09);
  }, [ensureAudio, soundEnabled]);
  const buttonFeedbackTimers = useRef(new WeakMap<HTMLButtonElement, number>());
  const showButtonPress = useCallback((target: EventTarget | null) => {
    const button = buttonFromTarget(target);
    if (!button || button.disabled) return;
    playButtonPress();
    const previousTimer = buttonFeedbackTimers.current.get(button);
    if (previousTimer) window.clearTimeout(previousTimer);
    button.classList.remove("button-pressed");
    void button.offsetWidth;
    button.classList.add("button-pressed");
    buttonFeedbackTimers.current.set(button, window.setTimeout(() => {
      button.classList.remove("button-pressed");
      buttonFeedbackTimers.current.delete(button);
    }, 220));
  }, [playButtonPress]);
  const pointerPressedButton = useRef<{button:HTMLButtonElement|null;at:number}|null>(null);
  const onButtonPointerDown = useCallback((event: ReactPointerEvent<HTMLElement>) => {
    pointerPressedButton.current = {button:buttonFromTarget(event.target),at:performance.now()};
    showButtonPress(event.target);
  }, [showButtonPress]);
  const onButtonClick = useCallback((event: ReactMouseEvent<HTMLElement>) => {
    const button = buttonFromTarget(event.target);
    const pointerPress = pointerPressedButton.current;
    if (!pointerPress || button !== pointerPress.button || performance.now()-pointerPress.at > 800) showButtonPress(event.target);
    pointerPressedButton.current = null;
  }, [showButtonPress]);
  ...
  useEffect(() => {
    const gameKey = `${game.sessionId}:${game.gameNumber}`, highest = Math.max(...game.players.map(player => player.stash));
    const tied = game.players.filter(player => player.stash === highest).map(player => player.id);
    if (leaderGame.current !== gameKey) {
      const initialLeader = tied.length === 1 ? tied[0] : null;
      leaderGame.current = gameKey; currentLeader.current = initialLeader; setLeaderId(initialLeader); setLeadFlash(null); return;
    }
    const previous = currentLeader.current;
    const next = previous !== null && tied.includes(previous) ? previous : tied.length === 1 ? tied[0] : null;
    if (previous === null) {
      if (next === null) return;
      currentLeader.current = next; setLeaderId(next);
      const flash = { id: ++leadSequence.current, playerId: next };
      setLeadFlash(flash); playLeadChange();
      window.setTimeout(() => setLeadFlash(current => current?.id === flash.id ? null : current), 1800);
      return;
    }
    if (next === null) { currentLeader.current = null; setLeaderId(null); return; }
    if (next === previous) return;
    currentLeader.current = next; setLeaderId(next);
    const flash = { id: ++leadSequence.current, playerId: next };
    setLeadFlash(flash); playLeadChange();
    window.setTimeout(() => setLeadFlash(current => current?.id === flash.id ? null : current), 1800);
  }, [game.players, game.sessionId, game.gameNumber, playLeadChange]);
  const startGame = useCallback(() => {
    if (remoteRef.current) {
      if (remoteRef.current.isHost) remoteRef.current.send("begin");
      return;
    }
  ...
  const finalOnboardingSlide = onboardingSlides.length, onboarding = onboardingSlides[onboardingSlide];

  return <main className="game-shell" onPointerDownCapture={onButtonPointerDown} onClickCapture={onButtonClick}>
    ...
    <section className="game-board">
      {game.players.map(p => { const activeGood = game.greaterGood.find(effect => effect.ownerId === p.id), scorePulse = scorePulses.find(pulse => pulse.playerId === p.id), bankedAgainstPlayer = game.players.some(owner => owner.armed.steal?.targetId === p.id), tookLead = leadFlash?.playerId === p.id, isLeader = leaderId === p.id; return <article key={p.id} className={`player-card seat-${p.id} ${p.shielded ? "shield-protected" : ""} ${p.style} ${p.id === youId ? "is-you" : ""} ${scorePulse ? `score-${scorePulse.direction}` : ""} ${tookLead ? "lead-change" : ""}`}>{isLeader && <div className={`lead-change-notice ${tookLead ? "celebrating" : ""}`} aria-label={`${p.name} is the leader`}><Music2/><b>LEAD!</b></div>}<div className="avatar">{p.name[0]}</div>{p.shielded && <div className={`shield-duration ${p.shieldUntil-now<=1000?"ending":""}`} style={{background:`conic-gradient(currentColor ${Math.max(0,(p.shieldUntil-now)/SHIELD_ACTIVE_MS)*360}deg,transparent 0deg)`,animationPlayState:paused?"paused":"running"}} aria-label={`Shield active for ${seconds(p.shieldUntil-now)}`}><ShieldCheck size={25}/></div>}<div className="player-main"><span>{p.id===youId?"YOU":p.approach}</span><h2>{p.name}<ShieldCheck className={`stash-shield ${p.shielded ? "ready" : "inactive"}`} size={19} aria-label={p.shielded ? "Shield active" : p.cooldowns.shield > now ? "Shield recharging" : "Shield not armed"}/><Zap className={`smash-status ${bankedAgainstPlayer ? "threatened" : "clear"}`} size={19} aria-label={bankedAgainstPlayer ? `Smash threatened: Steal banked against ${p.name}` : `Smash clear: no Steal banked against ${p.name}`}/></h2><div className="score-wrap"><strong key={scorePulse?.id ?? `score-${p.id}`} className={`score-value ${scorePulse ? `score-${scorePulse.direction}` : ""}`}>{money(p.stash)}</strong>{scorePulse && <i key={`delta-${scorePulse.id}`} className={`score-delta ${scorePulse.direction}`} aria-label={`${p.name} ${scorePulse.direction === "gain" ? "gained" : "lost"} ${money(Math.abs(scorePulse.amount))}`}>{moneyDelta(scorePulse.amount)}</i>}</div></div><div className="player-state">{!remote && <span><Radio size={12}/> {p.latency} ms</span>}{p.tacticsUntil > now && <span><Clock3 size={12}/> Tactics {seconds(p.tacticsUntil - now)}</span>}{p.cooldowns.shield > now && !p.shielded && <span><Clock3 size={12}/> Shield {seconds(p.cooldowns.shield - now)}</span>}{p.armed.steal && <span className="player-power-state" role="img" aria-label={`${p.name}'s Steal targeting ${game.players[p.armed.steal.targetId].name}, ${seconds(p.armed.steal.expiresAt-now)} remaining`} title={`Steal → ${game.players[p.armed.steal.targetId].name} · ${seconds(p.armed.steal.expiresAt-now)}`}><PowerUpMeter type="steal" progress={(p.armed.steal.expiresAt-now)/STEAL_ACTIVE_MS} size={28}/></span>}{activeGood && <span className="player-power-state" role="img" aria-label={`${p.name}'s Greater Good targeting ${game.players[activeGood.targetId].name}, ${seconds(activeGood.nextTick-now+(activeGood.remaining-1)*GOOD_TICK_MS)} remaining`} title={`Greater Good → ${game.players[activeGood.targetId].name}`}><PowerUpMeter type="good" progress={(activeGood.nextTick-now+(activeGood.remaining-1)*GOOD_TICK_MS)/(GOOD_TICKS*GOOD_TICK_MS)} size={28}/></span>}</div><ThreatMarkers players={game.players} drains={game.greaterGood} targetId={p.id} now={now}/></article>; })}
      <div className="vault-area">{doubleNotice && <div className={`double-celebration ${doubleNotice.type === "double_bonus_cancelled" ? "cancelled" : ""}`} role="status" style={{animationPlayState: paused ? "paused" : "running"}}><b>{doubleNotice.type === "double_bonus" ? "💰 DOUBLE DOWN · 2×!" : "DOUBLE DOWN · BONUS LOST"}</b><strong>{String(doubleNotice.details?.winner)}{doubleNotice.type === "double_bonus" ? ` wins ${exactCash(doubleNotice.details?.total)}` : " was intercepted"}</strong><span>{doubleNotice.type === "double_bonus" ? `${exactCash(doubleNotice.details?.base)} pot + ${exactCash(doubleNotice.details?.bonus)} comeback bonus` : "Steal takes the pot · bonus cancelled"}</span></div>}{paused && <div className="pause-banner"><Pause size={18}/> {remote ? (remote.isHost ? "PAUSED · HOST CAN RESUME" : "PAUSED · WAITING FOR HOST") : "PAUSED · PRESS SPACE TO RESUME"}</div>}{lockdownEvent && (lockedDown || now < game.lockdownUntil + 1400) && <LockdownEffects key={lockdownEvent.at} until={game.lockdownUntil} now={now} paused={paused} actor={String(lockdownEvent.details?.actor)} cleared={(lockdownEvent.details?.cleared ?? []) as {owner:string;target:string;action:string}[]} names={game.players.map(p=>p.name)}/>}{game.claims.length > 0 && <div className="contest-banner"><Radio size={16}/> TRUTH KEEPER RESOLVING…</div>}{recentNotice && <div className="resolution-banner"><Ban size={20}/><span>{recentNotice.message}</span></div>}<div className={`vault ${lockedDown ? "vault-in-lockdown" : ""}`} style={{animationPlayState:paused ? "paused" : "running"}}><div className="vault-glow"/><span>{started ? "CURRENT POT" : "READY POT"}</span><strong>{money(game.pot)}</strong><small>{started ? `${money(Math.max(0, cap - game.pot))} until automatic split` : remote && !remote.isHost ? "Waiting for the host to start" : "Start when everyone can see the board"}</small></div>{!started ? <section className="how-to-play-popup onboarding" role="dialog" aria-labelledby="how-to-play-title" aria-describedby="how-to-play-rules">
        <div className="onboarding-progress" role="progressbar" aria-label="Power-up introduction progress" aria-valuemin={1} aria-valuemax={finalOnboardingSlide + 1} aria-valuenow={onboardingSlide + 1}>{Array.from({length:finalOnboardingSlide + 1},(_,index)=><i key={index} className={index===onboardingSlide?"current":index<onboardingSlide?"seen":""}/>)}</div>
        {onboarding ? <><span className="eyebrow power-up-name">{onboarding.eyebrow}</span><div className="onboarding-icon"><PowerUpMeter type={onboarding.action} progress={1} size={104}/></div><h2 id="how-to-play-title">{onboarding.title}</h2><p id="how-to-play-rules">{onboarding.copy}</p></> : <><span className="eyebrow">READY TO SMASH?</span><div className="onboarding-icon-stack" aria-hidden="true">{onboardingSlides.map(slide=><PowerUpIcon key={slide.action} type={slide.action} size={42} title={false}/>)}</div><h2 id="how-to-play-title">First to {money(goal)} wins</h2><p id="how-to-play-rules">The pot grows automatically. Smash to claim it, then use your power-ups to control the table.</p></>}
        <div className="onboarding-controls"><button className="carousel-nav" disabled={onboardingSlide===0} onClick={()=>setOnboardingSlide(index=>Math.max(0,index-1))} aria-label="Previous rule"><ChevronLeft/> Previous</button>{onboardingSlide<finalOnboardingSlide?<button className="carousel-nav next" onClick={()=>setOnboardingSlide(index=>Math.min(finalOnboardingSlide,index+1))}>Next <ChevronRight/></button>:<button className="start-button" disabled={!!remote && !remote.isHost} onClick={startGame}><Play/> {remote && !remote.isHost ? "Waiting for Host to Start the Game" : "Start Game"}</button>}</div>
      </section> : <button className={`smash-button ${mainAction === "double" ? "double-ready" : ""}`} disabled={paused || lockedDown || (mainAction === "smash" && you.cooldowns.smash > now) || game.winner !== null} onClick={() => act(mainAction)}><Zap/> {mainAction === "double" ? "DOUBLE DOWN · 2×" : you.cooldowns.smash > now ? `SMASH · ${seconds(you.cooldowns.smash - now)}` : "SMASH"}</button>}{champion && <div className="victory">🏆 {champion.name} wins!</div>}</div>
    </section>
    <section className="control-dock"><div className="section-label"><span>YOUR ACTIONS</span><small>{!started ? "Start the game when ready" : paused ? "Paused · inspect the board" : lockedDown ? "Lockdown · get ready for GO" : tacticsLocked ? `Tactics timer: ${seconds(you.tacticsUntil - now)} · Smash remains available` : "Both tracks ready"}</small></div><div className="action-row">{actionIds.map(action => {
      const armed = action === "steal" && you.armed.steal, ownedGood = action === "good" ? game.greaterGood.find(effect => effect.ownerId === you.id) : undefined, activeGood = !!ownedGood, cooling = you.cooldowns[action] > now, shieldAlready = action === "shield" && you.shielded, targetHasNoCash = action === "good" && selectedTarget.stash <= 0, targetAlreadyGood = action === "good" && game.greaterGood.some(effect => effect.targetId === targetId);
      ...
      const duration = action === "lockdown" ? LOCKDOWN_COOLDOWN_MS : action === "steal" ? armed ? STEAL_ACTIVE_MS : actionCooldown.steal : action === "good" ? activeGood ? GOOD_TICKS * GOOD_TICK_MS : actionCooldown.good : shieldAlready ? SHIELD_ACTIVE_MS : SHIELD_COOLDOWN_MS;
      const remaining = action === "steal" && armed ? armed.expiresAt-now : action === "good" && ownedGood ? ownedGood.nextTick-now+(ownedGood.remaining-1)*GOOD_TICK_MS : action === "shield" && shieldAlready ? you.shieldUntil-now : Math.max(0,you.cooldowns[action]-now);
      const progress = remaining > 0 ? remaining/duration : 1, meterLabel = remaining > 0 ? `${actionMeta[action].label} ${armed || activeGood || shieldAlready ? "active" : "cooldown"}, ${seconds(remaining)} remaining` : `${actionMeta[action].label} ready`;
      return <button key={action} className={`power-control ${action} ${armed || activeGood || shieldAlready ? "armed" : ""}`} disabled={disabled} onClick={() => act(action)} aria-label={`${actionMeta[action].label}. ${detail}${targeted.has(action) ? `. Target: ${selectedTarget.name}` : ""}`}><PowerUpMeter type={action as PowerUpId} progress={progress} size={70} label={meterLabel}/><span className="power-control-copy"><b>{actionMeta[action].label}</b><small>{detail}</small></span></button>;
